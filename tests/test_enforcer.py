...
def test_deny_logged_to_session_log(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "sys.stdin",
        io.StringIO(
            _hook("Bash", {"command": "rm -rf dist"}, str(tmp_path), session_id="sess-deny")
        ),
    )
    with pytest.raises(SystemExit):
        run_enforce()
...
def test_git_tag_command_matches_tag_rule():
    ...
    assert _match_confirmation_text("Bash", "git tag v0.10.1", _TAG_RULE_TEXT) is True
...
def test_post_tool_use_writes_session_log(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        io.StringIO(
            _post_hook(
                "Bash", str(tmp_path), session_id="sess-99", tool_use_id="tuid-abc", duration_ms=250
            )
        ),
    )
    with pytest.raises(SystemExit) as exc:
...
def test_post_tool_use_skips_enforcement(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        mod, "check_prohibited", lambda *a, **kw: called.append(True) or orig(*a, **kw)
    )
    with pytest.raises(SystemExit) as exc:
        run_enforce()
    assert exc.value.code == 0
